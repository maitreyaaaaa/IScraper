...
async function processImportJobs({
  ...
  while (true) {
    if (maxJobs && processed.length >= maxJobs) break;
    const batchLimit = maxJobs ? Math.min(concurrency, maxJobs - processed.length) : concurrency;
    const batch = typeof store.claimNextJobs === 'function'
      ? await store.claimNextJobs({ userId, importId, limit: batchLimit, leaseOwner, leaseMs, perUserConcurrency })
...
async function processOneJob({
  ...
  try {
    ...
    if (mediaAnalysisEnabled && requiresMediaAnalysis(item) && !mediaPaths.length) {
      throw new Error('Media file could not be downloaded for analysis.');
    }

    const mediaAnalysis = mediaAnalysisEnabled && analysisPlan.mediaCredential
    ...
    return store.updateJob(userId, job.id, {
      status: 'done',
      error: null,
      leaseOwner: null,
      leaseExpiresAt: null,
...
async function chooseAnalysisPlan({
  ...
  const needsMedia = mediaAnalysisEnabled && requiresMediaAnalysis(item);
