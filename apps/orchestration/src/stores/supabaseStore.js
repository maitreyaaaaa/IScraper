...
const { initialIndexingStage: defaultIndexingStage, normalizeIndexingStage } = require('../services/indexingStages');
...
function createSupabaseStore({ url, serviceRoleKey }) {
  ...
  return {
    ...
    async createImport({ userId, source, mode = 'export', fileNames = [], status = 'imported', storageFiles = [], error: importError = null }) {
      const row = { user_id: userId, source, mode, file_names: fileNames, status };
      if (storageFiles.length || status !== 'imported') row.storage_files = storageFiles;
      if (importError || status !== 'imported') row.error = importError;
      const { data, error } = await client
        .from('imports')
        .insert(row)
        .select('*')
    ...
    async upsertImportData({ userId, importId, parsed, initialStatus = 'done', initialIndexingStage = defaultIndexingStage(initialStatus) }) {
    ...
    async claimStorageImport(id) {
      const { data, error } = await client
        .from('imports')
        .update({ status: 'processing_storage', error: null, updated_at: new Date().toISOString() })
        .eq('id', id)
        .eq('status', 'queued_storage')
        .select('*')
        .maybeSingle();
    ...
    async updateImportStatus(id, status, importError = null) {
      const { data, error } = await client
        .from('imports')
        .update({ status, error: importError, updated_at: new Date().toISOString() })
        .eq('id', id)
        .select('*')
        .maybeSingle();
      if (error) throw error;
...
function mapItem(row) {
  return {
    ...
    status: row.status,
    error: row.error,
    indexingStage: normalizeIndexingStage(row.indexing_stage, defaultIndexingStage(row.status)),
    indexingError: row.indexing_error || null,
    indexedTextAt: row.indexed_text_at || null,
    indexedVisualAt: row.indexed_visual_at || null,
    lastEnrichmentRequestedAt: row.last_enrichment_requested_at || null,
  };
}
...
function toSavedItemPatch(patch = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'indexingStage')) row.indexing_stage = patch.indexingStage;
  if (Object.prototype.hasOwnProperty.call(patch, 'indexingError')) row.indexing_error = patch.indexingError;
  if (Object.prototype.hasOwnProperty.call(patch, 'indexedTextAt')) row.indexed_text_at = patch.indexedTextAt;
  if (Object.prototype.hasOwnProperty.call(patch, 'indexedVisualAt')) row.indexed_visual_at = patch.indexedVisualAt;
  if (Object.prototype.hasOwnProperty.call(patch, 'lastEnrichmentRequestedAt')) row.last_enrichment_requested_at = patch.lastEnrichmentRequestedAt;
  return row;
}
