...
function seedFromLegacyIndex(dataPath) {
  ...
  return {
    ...
    items: legacy.map((item) => ({
      ...
      indexingStage: item.error ? 'index_failed' : 'text_indexed',
      indexingError: item.error || null,
      indexedTextAt: now(),
      indexedVisualAt: null,
      lastEnrichmentRequestedAt: null,
...
function normalizeState(state) {
  ...
  normalized.items = (normalized.items || []).map((item) => ({
    ...item,
    indexingStage: normalizeIndexingStage(item.indexingStage, defaultIndexingStage(item.status)),
    indexingError: item.indexingError || null,
    indexedTextAt: item.indexedTextAt || null,
    indexedVisualAt: item.indexedVisualAt || null,
    lastEnrichmentRequestedAt: item.lastEnrichmentRequestedAt || null,
  }));
  return normalized;
}
...
function createLocalStore({ dataPath }) {
  ...
  return {
    ...
    createImport({ userId, source, mode = 'export', fileNames = [], status = 'imported', storageFiles = [], error = null }) {
      const entry = {
        id: `import-${Date.now()}`,
        userId,
        source,
    ...
    upsertImportData({ userId, importId, parsed, initialStatus = 'done', initialIndexingStage = defaultIndexingStage(initialStatus) }) {
      ...
      for (const item of parsed.items) {
        ...
        if (existing) {
          Object.assign(existing, {
            ...
            indexingStage: normalizeIndexingStage(existing.indexingStage, initialIndexingStage),
            updatedAt: now(),
          });
          continue;
        }
        ...
        const created = {
          ...
          indexingStage: normalizeIndexingStage(initialIndexingStage, initialIndexingStage),
          indexingError: null,
          indexedTextAt: initialIndexingStage === 'text_indexed' ? now() : null,
          indexedVisualAt: null,
          lastEnrichmentRequestedAt: null,
          analysis: null,
          createdAt: now(),
