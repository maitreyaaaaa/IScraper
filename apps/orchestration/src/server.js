...
async function runSearch({ store, config, userId, query, filters = {} }) {
  void config;
  return store.search(userId, query, filters, { queryEmbedding: null });
}

function enrichmentCandidates(results, limit = 3) {
  return results
    .filter((item) => !['visual_indexing', 'visual_indexed', 'deep_indexed'].includes(item.indexingStage))
    .slice(0, limit)
    .map((item) => item.id);
}
...
function createApp({ store, config = {} }) {
  ...
  app.post('/api/items/:id/approve', asyncRoute(async (req, res) => {
    ...
    item = await store.updateSavedItem(req.user.id, item.id, {
      ...
      status: 'done',
      error: null,
      indexingStage: initialIndexingStage('done'),
      indexingError: null,
  ...
  app.post('/api/items/:id/enrich', searchRateLimit, asyncRoute(async (req, res) => {
    await requireCompletedProfile(req, store);
    const result = await enrichSavedItem({
      store,
      config,
      userId: req.user.id,
      itemId: req.params.id,
      force: req.body?.force === true,
      allowMedia: req.body?.allowMedia !== false,
  ...
  app.post('/api/imports/storage', importRateLimit, asyncRoute(async (req, res) => {
    ...
    const storageFiles = files.map((file) => validateStorageFile(file, {
      userId: req.user.id,
      maxUploadFileSizeBytes: config.maxUploadFileSizeBytes || 25 * 1024 * 1024,
    }));
    const importEntry = await store.createImport({
      userId: req.user.id,
    ...
    const processed = await processStorageImport({ store, config, importEntry });

    res.json({
      import: importEntry,
      importQueued: false,
      message: 'Import parsed and searchable from metadata.',
      itemCount: processed.items || 0,
      totalItemCount: processed.totalItems || 0,
      newItemCount: processed.items || 0,
  ...
  app.post('/api/saves/link', importRateLimit, asyncRoute(async (req, res) => {
    ...
    const initialStatus = req.body?.startProcessing === true ? 'done' : 'needs_review';
  ...
  app.post('/api/search', searchRateLimit, asyncRoute(async (req, res) => {
    ...
    res.json({ results, suggestedEnrichmentIds: enrichmentCandidates(results) });
  }));

  app.post('/api/enrichment/intent-batch', searchRateLimit, asyncRoute(async (req, res) => {
    await requireCompletedProfile(req, store);
    const itemIds = Array.isArray(req.body?.itemIds) ? req.body.itemIds : [];
    const results = await enrichIntentBatch({
      store,
      config,
      userId: req.user.id,
      itemIds,
