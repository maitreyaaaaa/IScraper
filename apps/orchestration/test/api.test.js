...
test('POST /api/imports imports all uploaded export files as metadata-searchable items', async () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'insta-brain-'));
  const store = createLocalStore({ dataPath: dir });
  const app = createApp({ store });
  const server = app.listen(0);
  ...
  try {
    ...
    assert.equal(store.getItems('local-dev-user').every((item) => item.indexingStage === 'text_indexed'), true);
  } finally {
    await new Promise((resolve) => server.close(resolve));
    rmSync(dir, { recursive: true, force: true });
  }
...
test('POST /api/imports skips already imported canonical duplicate URLs', async () => {
  ...
  try {
    ...
    assert.equal(existingItem.indexingStage, 'text_indexed');
  } finally {
    await new Promise((resolve) => server.close(resolve));
    rmSync(dir, { recursive: true, force: true });
  }
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/api/imports/storage`, {
      ...
      body: JSON.stringify({
        files: [
          {
            path: 'local-dev-user/imports/batch/saved_posts.html',
    ...
    assert.equal(body.error, 'Supabase storage is required for large upload imports.');
    const imports = store.dump().imports;
    assert.equal(imports.length, 1);
    assert.equal(imports[0].status, 'failed');
    assert.equal(imports[0].storageFiles[0].path, 'local-dev-user/imports/batch/saved_posts.html');
  } finally {
    await new Promise((resolve) => server.close(resolve));
    rmSync(dir, { recursive: true, force: true });
  }
...
test('POST /api/items/:id/approve makes a reviewed web save searchable from metadata', async () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'insta-brain-'));
  const store = createLocalStore({ dataPath: dir });
  const app = createApp({ store });
  const server = app.listen(0);
  ...
  try {
    ...
    assert.equal(approved.item.status, 'done');
    assert.equal(approved.item.indexingStage, 'text_indexed');
...
test('POST /api/search returns metadata results without requiring AI calls', async () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'insta-brain-'));
  const store = createLocalStore({ dataPath: dir });
  const app = createApp({ store });
  const server = app.listen(0);
  ...
  try {
    ...
    await fetch(`http://127.0.0.1:${port}/api/saves/link`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        url: 'https://example.com/claude-notes',
        title: 'Claude prompt patterns',
        description: 'Saved notes about Claude workflows and tags',
    ...
    assert.equal(body.results[0].indexingStage, 'text_indexed');
    assert.deepEqual(body.suggestedEnrichmentIds, [body.results[0].id]);
  } finally {
    await new Promise((resolve) => server.close(resolve));
    rmSync(dir, { recursive: true, force: true });
  }
...
test('POST /api/items/:id/enrich is user-scoped and idempotent', async () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'insta-brain-'));
  const store = createLocalStore({ dataPath: dir });
  const app = createApp({ store });
  const server = app.listen(0);
  ...
  try {
    ...
    const saveResponse = await fetch(`http://127.0.0.1:${port}/api/saves/link`, {
      ...
      body: JSON.stringify({
        url: 'https://example.com/reel/claude',
        title: 'Claude reel',
        description: 'A reel about Claude agents',
        startProcessing: true,
      }),
    });
    const saved = await saveResponse.json();

    const forbidden = await fetch(`http://127.0.0.1:${port}/api/items/${saved.item.id}/enrich`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-user-id': 'user-b',
    ...
    const first = await fetch(`http://127.0.0.1:${port}/api/items/${saved.item.id}/enrich`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-user-id': 'user-a',
    ...
    const second = await fetch(`http://127.0.0.1:${port}/api/items/${saved.item.id}/enrich`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-user-id': 'user-a',
    ...
    assert.equal(firstBody.item.indexingStage, 'text_indexed');
    assert.equal(second.status, 200);
    assert.equal(secondBody.skipped, true);
    assert.equal(secondBody.item.indexingStage, 'text_indexed');
  } finally {
    await new Promise((resolve) => server.close(resolve));
    rmSync(dir, { recursive: true, force: true });
  }
